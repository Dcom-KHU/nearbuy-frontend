...
export default function Page({
  ...
  const [formValues, setFormValues] = useState<auctionGetData>();
  ...
  const submitHandler = async (d: any) => {
    const registeredData = {
      ...
      tag: d.tag.split(" "),
      startPrice: JSON.parse(d.startPrice),
      increasePrice: JSON.parse(d.increasePrice),
      deadline: new Date(d.date).getTime(),
    };

    await customAxios
      .patch("/api/post/auction", registeredData, {
        params: { id: params.id },
      })
      .then(async data => {
        // 이미지 업로드
  ...
  useEffect(() => {
    (async () => {
      const getData = await customAxios.get("/api/post/auction", {
        params: { id: params.id },
      });
  ...
  useEffect(() => {
    ...
    if (formValues) {
      reset({
        ...
        increasePrice: formValues.increasePrice,
        date: formatDate(formValues.deadline),
      });
  ...
  return (
    <>
      <WriteSellBlock>
        <div className="self-start pl-8 py-5 text-[20px]">경매 포스팅 수정</div>
